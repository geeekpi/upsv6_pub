...
def calculate_power(voltage, current):
    ...
    voltage_in_v = voltage / 1000.0
    current_in_a = current / 1000.0
    return voltage_in_v * current_in_a  # Result in watts
...
try:
    while True:
        ...
        input_voltage = registers["INPUT_VOLTAGE"]
        input_current = registers["INPUT_CURRENT"]
        output_voltage = registers["OUTPUT_VOLTAGE"]
        output_current = registers["OUTPUT_CURRENT"]
        
        input_power = calculate_power(input_voltage, input_current)
        output_power = calculate_power(output_voltage, output_current)
        ...
        # Print power values
        print(f"* INPUT_POWER: {GREEN}{input_power:.2f}W{END_COLOR}")
        print(f"* OUTPUT_POWER: {RED}{output_power:.2f}W{END_COLOR}")
